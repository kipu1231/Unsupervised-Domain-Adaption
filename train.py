...
if __name__ == '__main__':
    ...
    source = [svhn_loader, mnistm_loader]
    target = [mnistm_loader, svhn_loader]

    '''create directory to save trained model and other info'''
    paths = [ "svhn-mnistm", "mnistm-svhn"]
    for path in paths:
    ...
    for i in range(len(source)):
        ...
        for epoch in range(1, args.epoch + 1):
            ...
            for step in range(steps):
                ...
                ''' Calculate loss '''
                source_class_out, source_domain_out = model(imgs, lambd)
                ...
                ''' Set domain of target data to 1 '''
                target_imgsize = target_imgs.size(0)
                domains = torch.ones(target_imgsize).long()
