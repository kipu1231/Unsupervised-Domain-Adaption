...
if __name__ == '__main__':
    ...
    #source_cls = torch.ones((args.train_batch,)).long()
    #target_cls = torch.zeros((args.train_batch,)).long()
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        #source_data = iter(source)
        #target_data = iter(target)
        ...
        for idx, ((source_img, _),(target_img, _)) in enumerate(zip(source, target)):
            ...
            #source_img, _ = next(source_data)
            #target_img, _ = next(target_data)
            ...
            #print(pred_concat.size(0)/2)
            # if (pred_concat.size(0)/2) == source_cls.size(0):
            #     source_cls = torch.ones((int(0.5*pred_concat.size(0)),)).long()
            #     target_cls = torch.zeros((int(0.5*pred_concat.size(0)),)).long()
            #
            # if torch.cuda.is_available():
            #     source_cls = source_cls.cuda()
            #     target_cls = target_cls.cuda()
            # labels = torch.cat((source_cls, target_cls))
            #print(labels.size(0)/2)
